...
export default function Home(props: Props) {
  const { updateDocs, clearDocs, docPostingList } = props;
  ...
  const postStartIndexer = () => {
    ...
    const res = AxiosProxy.post('/iindex/indexer/', formd, {
  ...
  const postQuery = () => {
    ...
    if (
      ...
      session.defaultSession.cookies
        .get({ url: 'http://localhost:8000', name: 'csrftoken' })
        .then(cookies => {
          console.log(cookies);
          setCookie(cookies);
          return cookies;
    ...
    const res = AxiosProxy.post('/iindex/query/', formd, {
      withCredentials: true
    })
      .then(response => {
        console.log(response);
      ...
      <Container p="100px" l="10px">
        ...
        {!queryFetched ? null : (
          <Message>
            ...
            <div style={{ display: 'inline-block', flex: true }}>
              {docs.map(doc => {
                return (
                  <p style={{ display: 'inline-block', flex: true }} key={doc}>
                    {`${doc}, `}
                  </p>
                );
              })}
